"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.getOperateToken = void 0;
const camunda_8_credentials_from_env_1 = require("camunda-8-credentials-from-env");
const OAuthProviderImpl_1 = require("./OAuthProviderImpl");
let operateOAuthProvider;
function getOperateToken(userAgentString) {
    if (operateOAuthProvider === undefined) {
        const creds = (0, camunda_8_credentials_from_env_1.getOperateCredentials)();
        operateOAuthProvider = new OAuthProviderImpl_1.OAuthProviderImpl({
            userAgentString,
            audience: creds.ZEEBE_TOKEN_AUDIENCE,
            clientId: creds.ZEEBE_CLIENT_ID,
            clientSecret: creds.ZEEBE_CLIENT_SECRET,
            authServerUrl: creds.CAMUNDA_OAUTH_URL
        });
    }
    return operateOAuthProvider.getToken('OPERATE');
}
exports.getOperateToken = getOperateToken;
