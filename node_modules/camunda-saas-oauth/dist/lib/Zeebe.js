"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.getZeebeToken = void 0;
const camunda_8_credentials_from_env_1 = require("camunda-8-credentials-from-env");
const OAuthProviderImpl_1 = require("./OAuthProviderImpl");
let zeebeOAuthProvider;
function getZeebeToken(userAgentString) {
    if (zeebeOAuthProvider === undefined) {
        const creds = (0, camunda_8_credentials_from_env_1.getZeebeCredentials)();
        zeebeOAuthProvider = new OAuthProviderImpl_1.OAuthProviderImpl({
            userAgentString,
            audience: creds.ZEEBE_TOKEN_AUDIENCE,
            clientId: creds.ZEEBE_CLIENT_ID,
            clientSecret: creds.ZEEBE_CLIENT_SECRET,
            authServerUrl: creds.CAMUNDA_OAUTH_URL
        });
    }
    return zeebeOAuthProvider.getToken('ZEEBE');
}
exports.getZeebeToken = getZeebeToken;
